-- PointPro production schema baseline.
...
create table if not exists public.mining_sessions (
  ...
  started_at timestamptz not null default now(), stopped_at timestamptz,
  rate numeric(30,12) not null default 0.00124, boost_multiplier numeric(10,4) not null default 1,
  status text not null default 'active' check (status in ('active','stopped')), created_at timestamptz not null default now()
);
create table if not exists public.transactions (
  id uuid primary key default gen_random_uuid(), user_id uuid not null references public.profiles(id) on delete cascade,
  type text not null check (type in ('mining','reward','referral','deposit','withdrawal','transfer','exchange','adjustment')),
  amount numeric(30,12) not null, status text not null default 'completed' check (status in ('pending','completed','failed','reversed')),
...
create table if not exists public.tasks (
  ...
  reward numeric(30,12) not null default 0, task_type text not null default 'daily', active boolean not null default true,
  created_at timestamptz not null default now()
);
create table if not exists public.user_tasks (
  user_id uuid not null references public.profiles(id) on delete cascade, task_id uuid not null references public.tasks(id) on delete cascade,
  status text not null default 'pending' check (status in ('pending','completed','claimed')), completed_at timestamptz, claimed_at timestamptz,
...
drop policy if exists "balances own read" on public.balances;
create policy "balances own read" on public.balances for select using (auth.uid() = user_id);
drop policy if exists "mining own read" on public.mining_sessions;
...
drop policy if exists "user tasks own read" on public.user_tasks;
...
-- RPCs are defined by the later PointPro migrations and production database state.
