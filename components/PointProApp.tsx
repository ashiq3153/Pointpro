...
function Profile({p}:any){
 const [profile,setProfile]=useState<any>(null),[refCount,setRefCount]=useState(0),[refEarned,setRefEarned]=useState(0),[copied,setCopied]=useState(false),[notifyOpen,setNotifyOpen]=useState(false),[prefs,setPrefs]=useState({mining:true,reward:true,task:true,withdrawal:true});
 ...
 return <div className="space-y-5 pb-3">
  ...
  <Card title="Account & Security"><MenuRow icon={<ShieldCheck/>} text="Security"/><button type="button" onClick={()=>setNotifyOpen(v=>!v)} className="flex w-full items-center gap-3 border-b border-[#edf0f5] py-4 text-left"><Bell size={19} className="text-[#2f70f4]"/><b className="flex-1 text-sm">Notifications</b><ChevronRight size={17} className={notifyOpen?"rotate-90 text-[#2f70f4]":"text-[#a0a8b7]"}/></button>{notifyOpen&&<div className="rounded-2xl bg-[#f5f8fc] p-3"><div className="mb-3"><b className="text-sm">Notification Preferences</b><p className="mt-1 text-[11px] text-[#8993a6]">Choose which account alerts you want to receive.</p></div>{([["mining","Mining activity","Mining started, paused and completed"],["reward","Daily rewards","Daily reward availability and claims"],["task","Task rewards","New tasks and reward updates"],["withdrawal","Withdrawals","Withdrawal status updates"]] as const).map(([key,title,desc])=><div key={key} className="flex items-center gap-3 border-b border-[#e4e9f1] py-3 last:border-0"><div className="grid h-9 w-9 place-items-center rounded-xl bg-white text-[#2f70f4]"><Bell size={16}/></div><div className="min-w-0 flex-1"><b className="block text-xs">{title}</b><span className="block text-[10px] text-[#8993a6]">{desc}</span></div><button type="button" onClick={()=>setPrefs(v=>({...v,[key]:!v[key]}))} className={`relative h-6 w-11 rounded-full transition ${prefs[key]?"bg-[#2f70f4]":"bg-[#cbd2dd]"}`}><span className={`absolute top-1 h-4 w-4 rounded-full bg-white shadow transition ${prefs[key]?"left-6":"left-1"}`}/></button></div>)}</div>}<MenuRow icon={<CircleHelp/>} text="Support"/><MenuRow icon={<FileText/>} text="Terms & Privacy"/><button onClick={async()=>{await supabase.auth.signOut()}} className="flex w-full items-center gap-3 py-4 text-left text-[#e5485d]"><LogOut size={19}/><b className="text-sm">Logout</b></button></Card>
 </div>}
