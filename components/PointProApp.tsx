...
export default function PointProApp(){
 ...
 const toggleMining=async()=>{const next=!mining;if(!supabase||!userId){setMining(next);notify(next?"Mining started locally — connect Supabase to persist":"Mining paused");return}if(next){const {data:active}=await supabase.from("mining_sessions").select("id").eq("user_id",userId).eq("status","active").limit(1).maybeSingle();if(active){setMining(true);return}const {error}=await supabase.from("mining_sessions").insert({user_id:userId,rate:speed,status:"active"});if(error){notify("Could not start mining");return}setMining(true);notify("Mining started")}else{const {data,error:settleError}=await supabase.functions.invoke("settle-mining");if(settleError){notify("Could not save mining earnings");return}const earned=Number(data?.settled)||0;const {data:session}=await supabase.from("mining_sessions").select("id").eq("user_id",userId).eq("status","active").order("started_at",{ascending:false}).limit(1).maybeSingle();if(session){const {error}=await supabase.from("mining_sessions").update({status:"stopped",stopped_at:new Date().toISOString()}).eq("id",session.id);if(error){notify("Could not stop mining");return}}if(earned>0){setBalance(v=>v+earned);setToday(v=>v+earned)}setMining(false);notify("Mining stopped and earnings saved")}};
