...
function Tasks({p}:any){const [tasks,setTasks]=useState<any[]>([]);const [loading,setLoading]=useState(true);const load=async()=>{if(!supabase||!p.userId){setLoading(false);return}const {data}=await supabase.from("tasks").select("id,title,description,reward,task_type").eq("active",true).order("created_at",{ascending:false});if(data){const {data:ut}=await supabase.from("user_tasks").select("task_id,status").eq("user_id",p.userId);const map=new Map((ut||[]).map((x:any)=>[x.task_id,x.status]));setTasks(data.map((t:any)=>({...t,status:map.get(t.id)||"pending"})))}setLoading(false)};useEffect(()=>{load()},[p.userId]);const claim=async(t:any)=>{if(!supabase||!p.userId)return p.notify("Connect your account to claim rewards");if(t.status==="claimed")return;const {error}=await supabase.from("user_tasks").upsert({user_id:p.userId,task_id:t.id,status:"claimed",completed_at:new Date().toISOString(),claimed_at:new Date().toISOString()});if(error)return p.notify("Reward could not be claimed");await supabase.from("transactions").insert({user_id:p.userId,type:"reward",amount:Number(t.reward),status:"completed",metadata:{task_id:t.id,title:t.title}});await load();p.notify("Reward recorded: +"+Number(t.reward).toFixed(3)+" PP")};return <div className="space-y-5"><Title title="Tasks" sub="Complete tasks and earn more PP Coin."/><div className="flex gap-2 overflow-x-auto pb-1">{["Watch Ads","Daily Tasks","Social Tasks","Special"].map((x,i)=><span key={x} className={`whitespace-nowrap rounded-full border px-5 py-2.5 text-sm font-bold ${i===0?"border-[#2f70f4] bg-[#2f70f4] text-white":"border-[#e4e8f0] bg-white text-[#8993a6]"}`}>{x}</span>)}</div><Card>{loading?<div className="p-6 text-center text-sm text-[#8993a6]">Loading tasks…</div>:tasks.length===0?<div className="p-6 text-center text-sm text-[#8993a6]">No active tasks right now.</div>:tasks.map((t:any)=><Task key={t.id} icon={t.task_type==="referral"?<Users/>:t.task_type==="mining"?<Zap/>:<Gift/>} title={t.title} text={t.description||"Complete this task"} button={t.status==="claimed"?"Claimed":"+"+Number(t.reward).toFixed(3)+" PP"} onClick={()=>claim(t)}/> )}</Card></div>}
