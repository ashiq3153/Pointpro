...
export default function PointProApp(){
 ...
 useEffect(()=>{if(!mining||!supabase||!userId)return;const id=window.setInterval(async()=>{const {data,error}=await supabase.functions.invoke("settle-mining");if(!error&&data?.settled){const earned=Number(data.settled)||0;setBalance(v=>v+earned);setToday(v=>v+earned);}},15000);return()=>clearInterval(id)},[mining,userId]);
