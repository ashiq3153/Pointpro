...
function Mine({p}:any){
 ...
 return <div className="space-y-5">
  ...
  <section className="relative overflow-hidden rounded-[28px] bg-[#07182c] p-5 text-white shadow-[0_16px_38px_rgba(5,22,45,.22)]">
   <div className="absolute -right-10 -top-10 h-32 w-32 rounded-full border-[18px] border-[#2f9cff]/10"/>
   <div className="relative z-10 flex items-start justify-between"><div><p className="text-[10px] font-black tracking-[.18em] text-[#5dbaff]">BOOST CENTER</p><h2 className="mt-1 text-[23px] font-black">Mining Power</h2><p className="mt-1 text-xs text-slate-400">Activate a boost to increase your live production rate.</p></div><div className="grid h-12 w-12 place-items-center rounded-2xl bg-[#2f70f4]/15 text-[#5dbaff]"><Zap size={23}/></div></div>
   <div className="relative z-10 mt-5 rounded-2xl border border-white/10 bg-white/5 p-3"><div className="flex items-center justify-between"><span className="text-[9px] font-bold uppercase tracking-wider text-slate-400">Current mining rate</span><span className={`text-[10px] font-black ${p.boost?"text-[#56efa0]":"text-slate-400"}`}>{p.boost?"BOOST ACTIVE":"BASE RATE"}</span></div><b className="mt-1 block text-[22px] tabular-nums text-[#ffd447]">+{p.speed.toFixed(5)} <span className="text-[10px] text-slate-400">PP/s</span></b></div>
   <div className="relative z-10 mt-3 grid grid-cols-2 gap-3"><button onClick={()=>p.setBoost(true)} className={`rounded-2xl border p-4 text-left transition ${p.boost?"border-[#39df91]/40 bg-[#39df91]/10":"border-white/10 bg-white/5"}`}><div className="flex items-center justify-between"><span className="grid h-9 w-9 place-items-center rounded-xl bg-[#2f70f4]/15 text-[#5dbaff]"><Zap size={17}/></span><span className="text-[10px] font-black text-[#39df91]">+10%</span></div><b className="mt-3 block text-sm">Energy Boost</b><p className="mt-1 text-[10px] text-slate-400">Fast activation</p><span className={`mt-3 block rounded-xl py-2 text-center text-[10px] font-black ${p.boost?"bg-[#39df91]/15 text-[#39df91]":"bg-[#2f70f4] text-white"}`}>{p.boost?"ACTIVE":"ACTIVATE"}</span></button><button onClick={()=>p.setBoost(true)} className="rounded-2xl border border-white/10 bg-white/5 p-4 text-left"><div className="flex items-center justify-between"><span className="grid h-9 w-9 place-items-center rounded-xl bg-[#9d6cff]/15 text-[#c29cff]"><Zap size={17}/></span><span className="text-[10px] font-black text-[#c29cff]">+20%</span></div><b className="mt-3 block text-sm">Super Boost</b><p className="mt-1 text-[10px] text-slate-400">Higher mining power</p><span className="mt-3 block rounded-xl bg-[#24334a] py-2 text-center text-[10px] font-black text-slate-300">ACTIVATE</span></button></div>
   <p className="relative z-10 mt-3 text-center text-[9px] text-slate-500">Boost availability and duration can be connected to your reward/payment rules.</p>
  </section>
...
function Profile({p}:any){
 const [profile,setProfile]=useState<any>(null),[refCount,setRefCount]=useState(0),[refEarned,setRefEarned]=useState(0),[copied,setCopied]=useState(false),[notifyOpen,setNotifyOpen]=useState(false),[securityOpen,setSecurityOpen]=useState(false),[prefs,setPrefs]=useState({mining:true,reward:true,task:true,withdrawal:true});
 ...
 return <div className="space-y-5 pb-3">
  ...
  <section className="rounded-[28px] bg-white p-5 shadow-[0_10px_28px_rgba(31,51,86,.07)]"><div className="flex items-center justify-between"><div><p className="text-[10px] font-black tracking-[.16em] text-[#8993a6]">REFERRAL CENTER</p><h2 className="mt-1 text-[22px] font-black text-[#17233a]">Invite & Earn</h2><p className="mt-1 text-xs text-[#8993a6]">Share your referral link and grow your rewards.</p></div><div className="grid h-12 w-12 place-items-center rounded-2xl bg-[#eef5ff] text-[#2f70f4]"><Users size={24}/></div></div><div className="mt-4 grid grid-cols-2 gap-3"><div className="rounded-2xl bg-[#f5f8fc] p-3"><span className="text-[9px] font-bold uppercase text-[#8993a6]">Friends joined</span><b className="mt-1 block text-xl">{refCount}</b></div><div className="rounded-2xl bg-[#f5f8fc] p-3"><span className="text-[9px] font-bold uppercase text-[#8993a6]">Referral earned</span><b className="mt-1 block text-xl text-[#20b96c]">+{refEarned.toFixed(3)} PP</b></div></div><div className="mt-4 rounded-2xl border border-dashed border-[#cfd8e6] p-3"><span className="block text-[9px] font-black tracking-wider text-[#8993a6]">YOUR REFERRAL CODE</span><div className="mt-1 flex items-center justify-between gap-3"><b className="text-2xl tracking-widest text-[#17233a]">{code}</b><button onClick={copy} className="rounded-xl bg-[#eef5ff] px-3 py-2 text-[10px] font-black text-[#2f70f4]"><Copy size={14}/></button></div></div><button onClick={p.share} className="mt-3 flex w-full items-center justify-center gap-2 rounded-2xl bg-[#2f70f4] py-3.5 text-sm font-black text-white"><Share2 size={16}/> Share Referral Link</button><p className="mt-3 text-center text-[10px] text-[#8993a6]">Your referral rewards are credited automatically when eligible conditions are met.</p></section>
  <Card title="Account & Security"><button type="button" onClick={()=>setSecurityOpen(v=>!v)} className="flex w-full items-center gap-3 border-b border-[#edf0f5] py-4 text-left"><ShieldCheck size={19} className="text-[#20b96c]"/><div className="flex-1"><b className="block text-sm">Security</b><span className="text-[10px] text-[#8993a6]">Protect your account and wallet</span></div><ChevronRight size={17} className={securityOpen?"rotate-90 text-[#2f70f4]":"text-[#a0a8b7]"}/></button>{securityOpen&&<div className="rounded-2xl bg-[#f5f8fc] p-3"><div className="flex items-center gap-3 rounded-2xl bg-[#eaf8f0] p-3"><ShieldCheck className="text-[#20b96c]" size={21}/><div><b className="block text-xs text-[#176b43]">Account protected</b><span className="text-[10px] text-[#438263]">Use your authenticated session for wallet actions.</span></div></div><div className="mt-2 space-y-1"><div className="flex items-center justify-between border-b border-[#e4e9f1] py-3"><span className="text-xs font-semibold">Authentication</span><b className="text-[10px] text-[#20b96c]">SECURE</b></div><div className="flex items-center justify-between border-b border-[#e4e9f1] py-3"><span className="text-xs font-semibold">Wallet withdrawals</span><b className="text-[10px] text-[#20b96c]">PROTECTED</b></div><div className="flex items-center justify-between py-3"><span className="text-xs font-semibold">Session</span><b className="text-[10px] text-[#2f70f4]">ACTIVE</b></div></div><button type="button" onClick={()=>p.notify("For password changes, use the account recovery flow.")} className="mt-2 w-full rounded-xl bg-white py-2.5 text-[10px] font-black text-[#2f70f4]">Account Recovery</button></div>}<button type="button" onClick={()=>setNotifyOpen(v=>!v)} className="flex w-full items-center gap-3 border-b border-[#edf0f5] py-4 text-left"><Bell size={19} className="text-[#2f70f4]"/><b className="flex-1 text-sm">Notifications</b><ChevronRight size={17} className={notifyOpen?"rotate-90 text-[#2f70f4]":"text-[#a0a8b7]"}/></button>{notifyOpen&&<div className="rounded-2xl bg-[#f5f8fc] p-3"><div className="mb-3"><b className="text-sm">Notification Preferences</b><p className="mt-1 text-[11px] text-[#8993a6]">Choose which account alerts you want to receive.</p></div>{([["mining","Mining activity","Mining started, paused and completed"],["reward","Daily rewards","Daily reward availability and claims"],["task","Task rewards","New tasks and reward updates"],["withdrawal","Withdrawals","Withdrawal status updates"]] as const).map(([key,title,desc])=><div key={key} className="flex items-center gap-3 border-b border-[#e4e9f1] py-3 last:border-0"><div className="grid h-9 w-9 place-items-center rounded-xl bg-white text-[#2f70f4]"><Bell size={16}/></div><div className="min-w-0 flex-1"><b className="block text-xs">{title}</b><span className="block text-[10px] text-[#8993a6]">{desc}</span></div><button type="button" onClick={()=>setPrefs(v=>({...v,[key]:!v[key]}))} className={`relative h-6 w-11 rounded-full transition ${prefs[key]?"bg-[#2f70f4]":"bg-[#cbd2dd]"}`}><span className={`absolute top-1 h-4 w-4 rounded-full bg-white shadow transition ${prefs[key]?"left-6":"left-1"}`}/></button></div>)}</div>}<MenuRow icon={<CircleHelp/>} text="Support"/><MenuRow icon={<FileText/>} text="Terms & Privacy"/><button onClick={async()=>{await supabase.auth.signOut()}} className="flex w-full items-center gap-3 py-4 text-left text-[#e5485d]"><LogOut size={19}/><b className="text-sm">Logout</b></button></Card>
 </div>}
